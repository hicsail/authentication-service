...
export class UserTestUtil {
  ...
  async createDummyAdmins(): Promise<User[]> {
    ...
    const dummyAdminsInput = [
      { projectId: this.dummyProjects[0].id, username: 'admin0', fullname: 'real admin0', email: 'admin0@mail.com', password: await bcrypt.hash('6pD$y38^6HzFcT6P', this.HASH_ROUNDS), role: 1 },
  ...
  async createDummyUsers(): Promise<User[]> {
    ...
    const dummyUsersInput = [
      { projectId: this.dummyProjects[0].id, username: 'proj00-user0',  fullname: 'proj00 user0', email: 'user0@mail.com', password: await bcrypt.hash('dz$d0I05s4!AmIkN', this.HASH_ROUNDS) },
      { projectId: this.dummyProjects[0].id, username: 'proj00-user1', fullname: 'proj00 user0',email: 'user1@mail.com', password: await bcrypt.hash('06!i68UKef87eCUs', this.HASH_ROUNDS) },
      { projectId: this.dummyProjects[0].id, username: 'proj00-user2', fullname: 'proj00 user0',email: 'user2@mail.com', password: await bcrypt.hash('V$fd5i9tu@LIHMa8', this.HASH_ROUNDS) },
